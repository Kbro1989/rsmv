...
function resolveAlias(typename: string, parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    ...
    switch (typeof chunkdef) {
        ...
        case "object":
            if (chunkdef == null) {
                ...
                if (parserFunctions[chunkdef[0] as string]) {
                    return parserFunctions[chunkdef[0] as string](args, parent, typedef);
                }
            }
        default:
...
function opcodesParser(chunkdef: Record<string, unknown>, parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    let r: ChunkParser = {
        read(state) {
        ...
        write(state, value) {
            ...
            for (let key in value) {
                ...
                opt.parser.write(state, (value as any)[key]);
            }
            if (!hasexplicitnull) {
    ...
    let resolveReference = function (targetprop: string, name: string, childresolve: ResolvedReference): ResolvedReference {
...
function tuppleParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    let r: ChunkParser = {
        read(state) {
            let r: any[] = [];
    ...
    const resolveReference = function (index: number, name: string, child: ResolvedReference): ResolvedReference {
...
export function buildReference(name: string, container: ChunkParentCallback | null, startingpoint: ResolvedReference): ResolvedReference {
...
function refgetter(refparent: ChunkParentCallback | null, propname: string, resolve: (v: unknown, old: number) => number) {
    ...
    return {
        read(state: SharedEncoderState) {
            ...
            return (stack[stack.length - depth] as any)[propname];
        },
        write(state: SharedEncoderState, newvalue: number) {
            ...
            let stack = (hidden ? state.hiddenstack : state.stack);
            (stack[stack.length - depth] as any)[propname] = newvalue;
        }
    }
}
...
function structParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    ...
    let r: ChunkParser = {
        ...
        write(state, value) {
            if (typeof value != "object" || !value) { throw new Error("object expected"); }
            let hiddenvalue: Record<string, any> = {};
            state.stack.push(value);
            ...
            state.stack.pop();
            state.hiddenstack.pop();
            return value; // Return value for consistency
        },
        getTypescriptType(indent) {
            let r = "{\n";
            let newindent = indent + "\t";
...
function optParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    let r: ChunkParser = {
        ...
        write(state, value) {
            ...
            return undefined; // Explicitly return undefined if value is null
        },
        getTypescriptType(indent) {
    ...
    let resolveReference = function (name: string, child: ResolvedReference): ResolvedReference {
        return buildReference(name, parent, {
            stackdepth: child.stackdepth,
            resolve(v, old) {
...
function chunkedArrayParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    ...
    const resolveLength: ChunkParentCallback = function (prop, childresolve): ResolvedReference {
    ...
    const resolveReference = function (targetprop: string, name: string, childresolve: ResolvedReference): ResolvedReference {
        let result: ResolvedReference = {
            stackdepth: childresolve.stackdepth + 1,
            resolve(v, oldvalue) {
                if (typeof v != "object" || !v) { throw new Error("object expected"); }
...
function bufferParserValue(value: unknown, type: typeof BufferTypes[keyof typeof BufferTypes], scalartype: keyof typeof BufferTypes): Uint8Array | Int8Array | Int16Array | Uint16Array | Int32Array | Uint32Array | Float32Array {
    if (typeof value == "string") {
        if (scalartype == "hex") {
            ...
            let m = (value as string).match(/^buffer ([\w\[\]]+){([\d,\-\.]*)}$/);
...
function bufferParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    let r: ChunkParser = {
        read(state) {
            ...
            if (scalartype == "hex") { (array as any).toJSON = () => {
                let hex = [];
    ...
    const resolveLengthReference: ChunkParentCallback = function (name, child): ResolvedReference {
        return buildReference(name, parent, {
            stackdepth: child.stackdepth,
...
function arrayParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    ...
    const resolvePropReference = function (name, child): ResolvedReference {
        return buildReference(name, parent, {
            stackdepth: child.stackdepth,
            resolve(v, old) {
                if (!Array.isArray(v)) { throw new Error("array expected"); }
...
function arrayNullTerminatedParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    let r: ChunkParser = {
        read(state) {
            let r: any[] = [];
            let ctx = { $opcode: 0 };
    ...
    const resolveReference: ChunkParentCallback = function (name, child): ResolvedReference {
        if (name == "$opcode") {
            return {
...
function literalValueParser(constvalue: unknown): ChunkParser {
...
function referenceValueParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
...
function intAccumolatorParser(args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
    let r: ChunkParser = {
        read(state) {
...
const hardcodes: Record<string, (args: unknown[], parent: ChunkParentCallback, typedef: TypeDef) => ChunkParser> = {
    playeritem: function (): ChunkParser {
        return {
            read(state) {
            ...
            write(state, value) {
                ...
                if (value == 0) {
                    ...
                    state.dataView.setUint16(state.scan, (value == -1 ? 0xffff : value & 0xffff), false); // Big-endian
                    state.scan += 2;
                }
            },
    ...
    itemvar: function (args: unknown[]): ChunkParser {
        let type = args[0];
        ...
        return {
            read(state) {
                ...
                else if (type == "matcount") { return (ref as any)?.replaceMaterials?.length ?? 0; }
                else if (type == "colorcount") { return (ref as any)?.replaceColors?.length ?? 0; }
                else if (type == "modelcount") { return (ref as any)?.models.length; }
    ...
    buildnr: function (args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
        return {
            readConst(state) {
    ...
    match: function (args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
        let r: ChunkParser = {
            read(state) {
    ...
    footer: function (args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
        ...
        return {
            ...
            write(state, v) {
                ...
                state.buffer.set(state.buffer.subarray(oldscan, state.scan), state.endoffset - len); // Use subarray and set for Uint8Array
                state.scan = oldscan;
                state.endoffset -= len;
            },
            getTypescriptType(indent) {
    ...
    "tailed varushort": function (args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
            ...
            write(state, v) {
                ...
                while (v >= 0) {
                    ...
                    if (chunk < 0x80) {
                        ...
                        state.dataView.setUint16(state.scan, chunk | 0x8000, false); // Big-endian
                        state.scan += 2;
                    }
                    v -= chunk;
                }
    ...
    "legacy_maptile": function (args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
        return {
            read(state) {
                let res = {
                    flags: 0,
...
`
    ...
    scriptopt: function (args: unknown[], parent: ChunkParentCallback, typedef: TypeDef): ChunkParser {
        return {
            read(state) {
                let cali = state.args.clientScriptDeob as any | undefined; // ClientscriptObfuscation
                if (!cali) {
...
function getClientVersion(args: Record<string, unknown>): number {
    if (typeof args.clientVersion != "number") { throw new Error("client version not set"); }
    return args.clientVersion;
}

const numberTypes: Record<string, { read: (s: SharedEncoderState) => number, write: (s: SharedEncoderState, v: number) => void, min: number, max: number }> = {
    ubyte: {
    ...
    ushort: {
        read(s) { let r = s.dataView.getUint16(s.scan, false); s.scan += 2; return r; }, // Big-endian
        write(s, v) { s.dataView.setUint16(s.scan, v, false); s.scan += 2; }, // Big-endian
        min: 0, max: 2 ** 16 - 1
    },
    short: {
        read(s) { let r = s.dataView.getInt16(s.scan, false); s.scan += 2; return r; }, // Big-endian
        write(s, v) { s.dataView.setInt16(s.scan, v, false); s.scan += 2; }, // Big-endian
        min: -(2 ** 15), max: 2 ** 15 - 1
    },
    uint: {
        read(s) { let r = s.dataView.getUint32(s.scan, false); s.scan += 4; return r; }, // Big-endian
        write(s, v) { s.dataView.setUint32(s.scan, v, false); s.scan += 4; }, // Big-endian
        min: 0, max: 2 ** 32 - 1
    },
    int: {
        read(s) { let r = s.dataView.getInt32(s.scan, false); s.scan += 4; return r; }, // Big-endian
        write(s, v) { s.dataView.setInt32(s.scan, v, false); s.scan += 4; }, // Big-endian
        min: -(2 ** 31), max: 2 ** 31 - 1
    },

    uint_le: {
        read(s) { let r = s.dataView.getUint32(s.scan, true); s.scan += 4; return r; }, // Little-endian
        write(s, v) { s.dataView.setUint32(s.scan, v, true); s.scan += 4; }, // Little-endian
        min: 0, max: 2 ** 32 - 1
    },
    ushort_le: {
        read(s) { let r = s.dataView.getUint16(s.scan, true); s.scan += 2; return r; }, // Little-endian
        write(s, v) { s.dataView.setUint16(s.scan, v, true); s.scan += 2; }, // Little-endian
        min: 0, max: 2 ** 16 - 1
    },
    utribyte: {
        read(s) {
        ...
        write(s, v) {
            s.dataView.setUint8(s.scan, (v >>> 16) & 0xFF);
            s.dataView.setUint8(s.scan + 1, (v >>> 8) & 0xFF);
            s.dataView.setUint8(s.scan + 2, v & 0xFF);
            s.scan += 3;
        },
        min: 0, max: 2 ** 24 - 1
    },
    float: {
        read(s) { let r = s.dataView.getFloat32(s.scan, false); s.scan += 4; return r; }, // Big-endian
        write(s, v) { s.dataView.setFloat32(s.scan, v, false); s.scan += 4; }, // Big-endian
    ...
    varushort: {
        ...
        write(s, v) {
            if (v < 0x80) {
                ...
                s.dataView.setUint16(s.scan, v | 0x8000, false); // Big-endian
                s.scan += 2;
            }
        },
        min: 0, max: 2 ** 15 - 1
    ...
    varshort: {
        ...
        write(s, v) {
            if (v < 0x40 && v >= -0x40) {
                ...
                s.dataView.setInt16(s.scan, v | 0x8000, false); // Big-endian
                s.scan += 2;
            }
        },
        min: -(2 ** 14), max: 2 ** 14 - 1
    ...
    varuint: {
        read(s) {
            let firstWord = s.dataView.getUint16(s.scan, false); // Big-endian
            s.scan += 2;
            if ((firstWord & 0x8000) == 0) {
                return firstWord;
            } else {
                let secondWord = s.dataView.getUint16(s.scan, false); // Big-endian
                s.scan += 2;
        ...
        write(s, v) {
            if (v < 0x8000) {
                s.dataView.setUint16(s.scan, v, false); // Big-endian
                s.scan += 2;
            } else {
                s.dataView.setUint32(s.scan, (v | 0x80000000) >>> 0, false); // Big-endian
                s.scan += 4;
            }
        },
        min: 0, max: 2 ** 31 - 1
    ...
    varnullint: {
        ...
        write(s, v) {
            if (v == -1) {
                s.dataView.setUint16(s.scan, 0x7fff, false); // Big-endian
                s.scan += 2;
            } else if (v < 0x8000) {
                s.dataView.setUint16(s.scan, v, false); // Big-endian
                s.scan += 2;
    ...
    varint: {
        ...
        write(s, v) {
            if (v < 0x4000 && v >= -0x4000) {
                s.dataView.setUint16(s.scan, v & 0x7fff, false); // Big-endian
                s.scan += 2;
            } else {
                s.dataView.setInt32(s.scan, v | 0x800000, false); // Big-endian
                s.scan += 4;
            }
        },
        min: -(2 ** 30), max: 2 ** 30 - 1
