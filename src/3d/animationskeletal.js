...
exports.mountSkeletalSkeleton = mountSkeletalSkeleton;
exports.parseSkeletalAnimation = parseSkeletalAnimation;
...
function debugBezierCurve(x0, x1, x2, x3, x) {
...
async function mountSkeletalSkeleton(rootnode, cache, framebaseid) {
    let base = opdecoder_1.parse.framemaps.read(await cache.engine.getFileById(constants_1.cacheMajors.framemaps, framebaseid), cache.engine.rawsource);
    if (!base.skeleton) {
    ...
    // let binds: Matrix4[] = [];
    let rootbones = [];
    let tmp = new three_1.Matrix4();
    let prematrix = new three_1.Matrix4().makeScale(1, 1, -1);
    for (let [id, entry] of base.skeleton.entries()) {
        let bone = new three_1.Bone();
        let matrix = new three_1.Matrix4().fromArray(entry.bonematrix);
        bone.name = "bone_" + id;
    ...
    let childbind = new three_1.Matrix4().copy(rootnode.matrixWorld);
...
function debugkeyframes(data, times, axis) {
...
async function parseSkeletalAnimation(cache, animid) {
    let anim = opdecoder_1.parse.skeletalAnim.read(await cache.engine.getFileById(constants_1.cacheMajors.skeletalAnims, animid), cache.engine.rawsource);
    let convertedtracks = [];
    ...
    for (let index = 0; index < animtracks.length;) {
        ...
        if (tracktype.t == "translate") {
            convertedtracks.push(new three_1.VectorKeyframeTrack(`${bonename}.position`, times, data));
        }
        if (tracktype.t == "scale") {
            ...
            convertedtracks.push(new three_1.VectorKeyframeTrack(`${bonename}.scale`, times, data));
        }
        if (tracktype.t == "rotate") {
            ...
            convertedtracks.push(new three_1.QuaternionKeyframeTrack(`${bonename}.quaternion`, times, quatdata));
        }
    }
    let clip = new three_1.AnimationClip("anim_" + (Math.random() * 1000 | 0), undefined, convertedtracks);
