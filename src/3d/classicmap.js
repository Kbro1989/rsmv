...
exports.getClassicMapData = getClassicMapData;
exports.classicModifyTileGrid = classicModifyTileGrid;
exports.classicDecodeMaterialInt = classicDecodeMaterialInt;
exports.getClassicLoc = getClassicLoc;
exports.classicIntsToModelMods = classicIntsToModelMods;
exports.classicOverlays = classicOverlays;
exports.classicUnderlays = classicUnderlays;
...
async function getClassicMapData(engine, rs2x, rs2z) {
    ...
    for (let level = 0; level < nlevels; level++) {
        ...
        let heifile = (engine.getBuildNr() <= 115 ? undefined : await engine.findSubfileByName(0, classicloader_1.classicGroups.land, `m${sourcelevel}${chunknum}.hei`));
        let jmfile = await engine.findSubfileByName(0, classicloader_1.classicGroups.maps, `m${sourcelevel}${chunknum}.jm`);
    ...
    for (let level = 0; level < nlevels; level++) {
        ...
        if (!leveldata.jm) {
            let datfile = await engine.findSubfileByName(0, classicloader_1.classicGroups.maps, `M${leveldata.sourcelevel}${chunknum}.DAT`);
            let locfile = await engine.findSubfileByName(0, classicloader_1.classicGroups.maps, `M${leveldata.sourcelevel}${chunknum}.LOC`);
    ...
    for (let level = 0; level < nlevels; level++) {
        ...
        if (leveldata.jm) {
            ...
            mapfilehash = (0, crc32util_1.crc32)(leveldata.jm, mapfilehash);
        }
        else if (leveldata.hei) {
            grid.loadHeiFile(leveldata.hei, level);
            mapfilehash = (0, crc32util_1.crc32)(leveldata.hei, mapfilehash);
        }
    }
    ...
    for (let level = 0; level < nlevels; level++) {
        ...
        if (!leveldata.jm) {
            if (leveldata.dat) {
                ...
                mapfilehash = (0, crc32util_1.crc32)(leveldata.dat, mapfilehash);
            }
            if (leveldata.loc) {
                grid.loadLocFile(leveldata.loc, level);
                mapfilehash = (0, crc32util_1.crc32)(leveldata.loc, mapfilehash);
            }
            else {
                let locs = loadLocJsonBuffer(config, chunkx, chunkz, leveldata.sourcelevel);
                mapfilehash = (0, crc32util_1.crc32)(Buffer.from(locs.buffer), mapfilehash);
...
class ClassicMapBuilder {
    ...
    loadJmFile(jmfile, level) {
        let jm = new utils_1.Stream(jmfile);
        let lastTerrain = 0;
    ...
    loadHeiFile(heifile, level) {
        let hei = new utils_1.Stream(heifile);
    ...
    loadDatfile(datfile, level) {
        let dat = new utils_1.Stream(datfile);
    ...
    addWallBuffers(horbuffer, verbuffer, diag1buffer, diag2buffer, roofids, overlaybuffer, rotatebuffer, level) {
        ...
        let tryExtendBridge = (level, x, z, bridgeid, waterid) => {
    ...
    loadLocFile(locfile, level) {
        let loc = new utils_1.Stream(locfile);
...
function classicModifyTileGrid(grid) {
    ...
    let getoverlay = (tile) => (tile?.debug_raw?.overlay ? grid.engine.classicData.tiles[tile.debug_raw?.overlay - 1] : undefined);
    for (let level = 0; level < grid.levels; level++) {
        for (let z = grid.zsize - 1; z >= 1; z--) {
            for (let x = grid.xsize - 1; x >= 1; x--) {
                let tile = grid.getTile(grid.xoffset + x, grid.zoffset + z, level);
                ...
                if (tile && (overlay?.type.autoconnect || overlay?.type.indoors)) {
                    ...
                    if (overlay.blocked) {
                        if (tile.rawCollision) {
                            tile.rawCollision.walk[0] = true;
                        }
                        if (tile.effectiveCollision) {
                            tile.effectiveCollision.walk[0] = true;
                        }
                    }
                    ...
                    if (hastop && hasleft && !hasbot && !hasright) {
                        tile.shape = mapsquare_1.tileshapes[5];
                    }
                    if (hastop && !hasleft && !hasbot && hasright) {
                        tile.shape = mapsquare_1.tileshapes[6];
                    }
                    if (!hastop && !hasleft && hasbot && hasright) {
                        tile.shape = mapsquare_1.tileshapes[7];
                    }
                    if (!hastop && hasleft && hasbot && !hasright) {
                        tile.shape = mapsquare_1.tileshapes[4];
                    }
                }
            }
        }
...
function classicDecodeMaterialInt(int) {
    let material = 0;
    let invisible = false;
    ...
    return {
        color,
        colorint: (0, utils_1.HSL2packHSL)(...(0, utils_1.RGB2HSL)(...color)),
        material,
        invisible
    };
}
function getClassicLoc(engine, id) {
    ...
    if (id >= classicLocIdRoof) {
        ...
        locdata = {
            ...
            models: [
                { type: 10, values: [modeltothree_1.constModelsIds.classicRoof10] },
                { type: 12, values: [modeltothree_1.constModelsIds.classicRoof12] },
                { type: 13, values: [modeltothree_1.constModelsIds.classicRoof13] },
                { type: 14, values: [modeltothree_1.constModelsIds.classicRoof14] },
                { type: 15, values: [modeltothree_1.constModelsIds.classicRoof15] },
                { type: 16, values: [modeltothree_1.constModelsIds.classicRoof16] },
                { type: 17, values: [modeltothree_1.constModelsIds.classicRoof17] }
            ],
        ...
        locdata = {
            ...
            probably_morphFloor: true,
            models: [
                { type: 0, values: [modeltothree_1.constModelsIds.classicWall] },
                { type: 9, values: [modeltothree_1.constModelsIds.classicWallDiag] }
            ],
            //sets replace_colors/mats and if invisible sets models to null
...
async function classicOverlays(engine) {
    ...
    let texindex = await engine.findSubfileByName(0, classicloader_1.classicGroups.textures, "INDEX.DAT");
    let usetga = !texindex;
    return Promise.all(config.tiles.map(async (q) => {
        ...
        if (mods.material) {
            ...
            let img = await (0, legacycache_1.combineLegacyTexture)(engine, texmeta.name, texmeta.subname, usetga);
            let r = 0, g = 0, b = 0;
