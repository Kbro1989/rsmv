...
exports.parseRT2Model = parseRT2Model;
const three_1 = require("three");
const opdecoder_1 = require("../opdecoder");
function parseRT2Model(modelfile, source) {
    let parsed = opdecoder_1.parse.classicmodels.read(modelfile, source);
    const posscale = 4;
    ...
    for (let [matid, count] of matusecount.entries()) {
        matmeshes.set(matid, {
            pos: new three_1.BufferAttribute(new Float32Array(count.verts * 3), 3),
            normals: new three_1.BufferAttribute(new Float32Array(count.verts * 3), 3),
            color: new three_1.BufferAttribute(new Uint8Array(count.verts * 3), 3, true),
            texuvs: new three_1.BufferAttribute(new Float32Array(count.verts * 2), 2),
    ...
    let addvert = (group, verts, polyindex, color) => {
        ...
        if (group.matid == -1) {
            group.color.setXYZ(group.currentface, ((~color >> 10) & 0x1f) / 31, ((~color >> 5) & 0x1f) / 31, ((~color >> 0) & 0x1f) / 31);
        }
        else {
            group.color.setXYZ(group.currentface, 1, 1, 1);
        }
        group.pos.setXYZ(group.currentface, parsed.xpos[posindex] * posscale, -parsed.ypos[posindex] * posscale, parsed.zpos[posindex] * posscale);
        group.normals.setXYZ(group.currentface, currentNormal.x, currentNormal.y, currentNormal.z);
        group.texuvs.setXY(group.currentface, (polyindex == 0 || polyindex == 3 ? 0 : 1), (polyindex == 0 || polyindex == 1 ? 0 : 1));
        return group.currentface++;
    };
    let currentNormal = new three_1.Vector3();
    ...
    let r = {
        ...
        meshes: [...matmeshes.values()].map(q => {
            let indices = new three_1.BufferAttribute(q.index, 1);
            return {
                indices: indices,
                vertexstart: 0,
