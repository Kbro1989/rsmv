...
exports.parseRT5Model = parseRT5Model;
...
function parseRT5Model(modelfile, source) {
    ...
    let modeldata = opdecoder_1.parse.oldmodels.read(modelfile, source);
    let maxy = 0;
    let miny = 0;
    let bonecount = 0;
    let skincount = 0;
    ...
    let yvalue = 0;
    let zvalue = 0;
    let xstream = new utils_1.Stream(modeldata.posx);
    let ystream = new utils_1.Stream(modeldata.posy);
    let zstream = new utils_1.Stream(modeldata.posz);
    ...
    for (let i = 0; i < modeldata.texmapcount; i++) {
        ...
        textureMappings.push({
            texspace: new three_1.Matrix4(),
            vertexsum: new three_1.Vector3(),
            vertexcount: 0,
            vertexmin: new three_1.Vector3(10e6, 10e6, 10e6),
            vertexmax: new three_1.Vector3(-10e6, -10e6, -10e6),
            args: flag
        });
    }
    let matusecount = new Map();
    let matmesh = new Map();
    ...
    if (modeldata.modelversion >= 16) {
        usenewuvkey = 0x7fff;
        let uvstream = new utils_1.Stream(modeldata.uvs);
        while (!uvstream.eof()) {
    ...
    if (modeldata.mode_2) {
        ...
        for (let i = 0; i < modeldata.facecount; i++) {
            ...
            if (op & 2) {
                materialbuffer[i] = (0, utils_1.flipEndian16)((0, utils_1.flipEndian16)(colorbuffer[i]) + 1); //TODO fix the endian thing at the other spot
                colorbuffer[i] = (0, utils_1.flipEndian16)(127); //hsl for white//TODO unflip endianness
                uvids.push((op >> 2) + 1);
            }
        }
    }
    ...
    for (let [matid, facecount] of matusecount) {
        ...
        let mesh = {
            pos: new three_1.BufferAttribute(new Float32Array(finalvertcount * 3), 3),
            normals: new three_1.BufferAttribute(new Float32Array(finalvertcount * 3), 3),
            color: new three_1.BufferAttribute(new Uint8Array(finalvertcount * colstride), colstride, true),
            texuvs: new three_1.BufferAttribute(new Float32Array(finalvertcount * 2), 2),
            index: new Uint16Array(facecount * 3),
            currentface: 0,
            matid: (0, utils_1.flipEndian16)(matid) - 1 //TODO fix endianness elsewhere
        };
    ...
    let stream = new utils_1.Stream(modeldata.indexbuffer);
    for (let i = 0; i < modeldata.facecount; i++) {
    ...
    if (materialbuffer) {
        let posa = new three_1.Vector3();
        let posb = new three_1.Vector3();
        let posc = new three_1.Vector3();
        let texindex = 0;
        for (let i = 0; i < modeldata.facecount; i++) {
    ...
    if (modeldata.texflags) {
        let mtmp = new three_1.Matrix4();
        let v0 = new three_1.Vector3();
        let v1 = new three_1.Vector3();
        let v2 = new three_1.Vector3();
        let vtmp = new three_1.Vector3();
        ...
        for (let i = 0; i < modeldata.texflags.length; i++) {
            ...
            if (mapping.args.type == 0) {
                ...
                mapping.texspace.set(v1.x, v2.x, vtmp.x, v0.x, v1.y, v2.y, vtmp.y, v0.y, v1.z, v2.z, vtmp.z, v0.z, 0, 0, 0, 1);
                ...
                //     debugger;
                // }
                mapping.texspace.set(space[0], -space[1], space[2], 0, space[3], -space[4], space[5], 0, space[6], -space[7], space[8], 0, 0, 0, 0, 1);
            ...
            if (globalThis.testmat >= 0 && globalThis.testmat == i) {
                let geo2 = new three_1.LatheGeometry([new three_1.Vector2(0.05, 0), new three_1.Vector2(0.05, 1), new three_1.Vector2(0.15, 1), new three_1.Vector2(0, 1.15)]);
                let geo;
                if (mapping.args.type == 0) {
                    geo = new three_1.PlaneGeometry(1, 1);
                }
                else if (mapping.args.type == 1) {
                    geo = new three_1.CylinderGeometry(0.5, 0.5, 1, 10);
                }
                else if (mapping.args.type == 2) {
                    geo = new three_1.BoxGeometry(1, 1, 1);
                }
                else if (mapping.args.type == 3) {
                    geo = new three_1.SphereGeometry(1);
                }
                let mesh = new three_1.Mesh(geo, debugmat);
                let arrowmesh = new three_1.Mesh(geo2, debugmat2);
    ...
    let v1 = new three_1.Vector3();
    let v2 = new three_1.Vector3();
    let vnormal = new three_1.Vector3();
    let vtmp0 = new three_1.Vector3();
    let vtmp1 = new three_1.Vector3();
    let vtmp2 = new three_1.Vector3();
    let m3tmp = new three_1.Matrix3();
    for (let i = 0; i < modeldata.facecount; i++) {
        srcindex0 = vertexindex[i * 3 + 0];
        srcindex1 = vertexindex[i * 3 + 1];
        srcindex2 = vertexindex[i * 3 + 2];
        ...
        if (colorbuffer) {
            ...
            let [r, g, b] = (0, utils_1.HSL2RGBfloat)((0, utils_1.packedHSL2HSL)((0, utils_1.flipEndian16)(colint)));
            if (!modeldata.alpha) {
    ...
    let meshes = [...matmesh.values()].map(m => {
        let indices = new three_1.BufferAttribute(m.index, 1);
        return {
            indices: indices,
            vertexstart: 0,
    ...
    let r = { maxy, miny, meshes, bonecount: bonecount, skincount: skincount, debugmeshes };
    return r;
}
