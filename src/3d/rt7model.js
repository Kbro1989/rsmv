...
exports.getBoneCenters = getBoneCenters;
exports.getModelCenter = getModelCenter;
exports.parseOb3Model = parseOb3Model;
exports.makeModelData = makeModelData;
const utils_1 = require("../utils");
const THREE = __importStar(require("three"));
const gltfutil_1 = require("./gltfutil");
const opdecoder_1 = require("../opdecoder");
function streamChunk(constr, stream, length) {
    let buf = (0, gltfutil_1.alignedRefOrCopy)(constr, stream.getData(), stream.scanloc(), length);
...
function addUvBuffer(attributes, vertexCount, uvBuffer) {
    if (uvBuffer instanceof Uint16Array) {
        ...
        for (let i = 0; i < vertexCount * 2; i++) {
            uvBufferCopy[i] = (0, utils_1.ushortToHalf)(uvBuffer[i]);
        }
...
function parseOb3Model(modelfile, source) {
    let parsed = opdecoder_1.parse.models.read(modelfile, source);
    let meshes = [];
    if (parsed.meshes) {
        for (let mesh of parsed.meshes) {
            ...
            if (mesh.colourBuffer) {
                ...
                for (let i = 0; i < mesh.faceCount; i++) {
                    let [r, g, b] = (0, utils_1.HSL2RGB)((0, utils_1.packedHSL2HSL)(mesh.colourBuffer[i]));
        ...
        if (mesh.vertexColours) {
            ...
            for (let i = 0; i < mesh.vertexColours.length; i++) {
                let [r, g, b] = (0, utils_1.HSL2RGB)((0, utils_1.packedHSL2HSL)(mesh.vertexColours[i]));
...
function makeModelData(meshes) {
    ...
    let r = { maxy, miny, meshes, bonecount: bonecount, skincount: skincount };
    return r;
}
