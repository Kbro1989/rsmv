import { CacheFileSource, CacheIndex, SubFile } from "./cache/index";
...
export class ClassicFileSource extends CacheFileSource {
    ...
    static async create(files: ScriptFS, version?: DetectedVersion) {
        if (!version) {
            ...
            // @ts-ignore
            let index = globalThis.localStorage?.rsmv_classicversion ?? "-1";
...
export async function classicConfig(source: ClassicFileSource, buildnr: number) {
    ...
    let wallobjects = mapprops(getushort(), {
        ...
        command_0: getstring,
        command_1: getstring,
        wallheight: getushort, // renamed to avoid conflict
        frontdecor: getint,
        backdecor: getint,
        blocked: getbool,
        invisible: getbool
