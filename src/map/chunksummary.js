...
exports.RenderDepsTracker = exports.ImageDiffGrid = void 0;
exports.getLocImageHash = getLocImageHash;
exports.chunkSummary = chunkSummary;
exports.mapsquareFloorDependencies = mapsquareFloorDependencies;
exports.mapsquareLocDependencies = mapsquareLocDependencies;
exports.compareFloorDependencies = compareFloorDependencies;
exports.compareLocDependencies = compareLocDependencies;
exports.mapdiffmesh = mapdiffmesh;
exports.generateLocationHashBoxes = generateLocationHashBoxes;
exports.generateFloorHashBoxes = generateFloorHashBoxes;
exports.pointsIntersectProjection = pointsIntersectProjection;
const three_1 = require("three");
const mapsquare_1 = require("../3d/mapsquare");
const modeltothree_1 = require("../3d/modeltothree");
const modelutils_1 = require("../3d/modelutils");
const crc32util_1 = require("../libs/crc32util");
const utils_1 = require("../utils");
function getLocImageHash(grid, info) {
    ...
    for (let dfloor = 0; dfloor <= 1; dfloor++) {
        for (let dz = 0; dz <= sizex; dz++) {
            for (let dx = 0; dx <= sizex; dx++) {
                subgrid.push((0, mapsquare_1.getTileHeight)(grid, info.x + dx, info.z + dz, info.plane + dfloor));
            }
        }
    }
    let baseheight = (0, mapsquare_1.getTileHeight)(grid, info.x, info.z, info.plane);
    let hash = modelPlacementHash(info);
    for (let height of subgrid) {
        hash = (0, crc32util_1.crc32addInt)(height - baseheight, hash);
    }
    return hash;
}
const pointAttribute = new three_1.BufferAttribute(new Float32Array(3), 3);
function getLocCenter(grid, model) {
    let first = model[0];
    let sum = new three_1.Vector3();
    let tmp = new three_1.Vector3();
    sum.set(0, 0, 0);
    let count = 0;
    ...
    let newpos = (0, mapsquare_1.transformVertexPositions)(pointAttribute, first.morph, grid, first.maxy - first.miny, 0, 0);
    tmp.fromBufferAttribute(newpos, 0);
    tmp.divideScalar(mapsquare_1.tiledimensions);
    return tmp;
}
function chunkSummary(grid, locdefs, rect) {
    let locids = new Map();
    let locs = [];
    let hashes = new Map();
    for (let [locdata, model] of locdefs) {
        ...
        center.x -= locdata.x;
        center.y -= (0, mapsquare_1.getTileHeight)(grid, locdata.x, locdata.z, locdata.plane) / mapsquare_1.tiledimensions;
        center.z -= locdata.z;
        hashes.set(imghash, {
            center: [
                +center.x.toFixed(2),
            ...
            for (let level = 0; level < grid.levels; level++) {
                ...
                for (let dx = 0; dx < groupsize; dx++) {
                    for (let dz = 0; dz < groupsize; dz++) {
                        ...
                        if (!tile || (!tile.underlayVisible && !tile.overlayVisible)) {
                            continue;
                        }
                        ...
                        tilehash = (0, crc32util_1.crc32addInt)(rawtile.height ?? -1, tilehash);
                        tilehash = (0, crc32util_1.crc32addInt)(rawtile.overlay ?? -1, tilehash);
                        tilehash = (0, crc32util_1.crc32addInt)(rawtile.settings ?? -1, tilehash);
                        tilehash = (0, crc32util_1.crc32addInt)(rawtile.shape ?? -1, tilehash);
                        tilehash = (0, crc32util_1.crc32addInt)(rawtile.underlay ?? -1, tilehash);
                        tilehash = (0, crc32util_1.crc32addInt)(tile.effectiveVisualLevel, tilehash);
                        ...
                        for (let i = tile.effectiveVisualLevel; i < grid.levels; i++) {
                            tilehashes[i] = (0, crc32util_1.crc32addInt)(tilehash, tilehashes[i]);
                        }
                    }
                }
            }
...
function mapsquareLocDependencies(grid, deps, locs, chunkx, chunkz) {
    const boxAttribute = new three_1.BufferAttribute(new Float32Array(3 * 8), 3);
    const v0 = new three_1.Vector3();
    const v1 = new three_1.Vector3();
    const v2 = new three_1.Vector3();
    let locgroups = new Map();
    for (let loc of locs.keys()) {
        let group = (0, utils_1.getOrInsert)(locgroups, loc.locid, () => []);
        group.push(loc);
    }
    let outlocgroups = [];
    for (let [locid, group] of locgroups) {
        ...
        for (let loc of group) {
            ...
            let first = models[0];
            let trans = (0, mapsquare_1.transformVertexPositions)(boxAttribute, first.morph, grid, first.maxy, chunkx * mapsquare_1.rs2ChunkSize * mapsquare_1.tiledimensions, chunkz * mapsquare_1.rs2ChunkSize * mapsquare_1.tiledimensions);
...
function tileSetVertices(tile) {
    let x0 = tile.x * mapsquare_1.tiledimensions;
    let x1 = x0 + tile.xzsize * mapsquare_1.tiledimensions;
    let z0 = tile.z * mapsquare_1.tiledimensions;
    let z1 = z0 + tile.xzsize * mapsquare_1.tiledimensions;
    let y0 = 0;
    let y1 = tile.maxy;
    return [
        x0, y0, z0,
...
async function mapdiffmesh(scene, points, col = [255, 0, 0]) {
    let tri = (model, verts, a, b, c) => model.mat(-1).addTriangle(col, verts.slice(a * 3, a * 3 + 3), verts.slice(b * 3, b * 3 + 3), verts.slice(c * 3, c * 3 + 3));
    let models = new three_1.Group();
    models.matrixAutoUpdate = false;
    models.updateMatrix();
    for (let group of points) {
        let model = new modelutils_1.ModelBuilder();
        ...
        models.add(await (0, modeltothree_1.ob3ModelToThree)(scene, model.convert()));
    }
    return models;
}
async function generateLocationHashBoxes(scene, locs, grid, chunkx, chunkz, level) {
    ...
    for (let loc of locdeps) {
        for (let inst of loc.instances) {
            ...
            let color = [(totalhash >> 16) & 0xff, (totalhash >> 8) & 0xff, (totalhash >> 0) & 0xff];
...
async function generateFloorHashBoxes(scene, grid, chunk, level) {
    ...
    for (let dep of floordeps) {
        let totalhash = 0;
        totalhash = (0, crc32util_1.crc32addInt)(dep.dephash, totalhash);
        totalhash = (0, crc32util_1.crc32addInt)(dep.tilehashes[level], totalhash);
        let color = [(totalhash >> 16) & 0xff, (totalhash >> 8) & 0xff, (totalhash >> 0) & 0xff];
...
function pointsIntersectProjection(projection, points) {
    ...
    const min = new three_1.Vector3();
    const max = new three_1.Vector3();
...
class ImageDiffGrid {
    ...
    calculateDiffArea(imgwidth, imgheight) {
        const boxtemp = new three_1.Box2();
        const d0 = new three_1.Vector2();
        const d1 = new three_1.Vector2();
        const d2 = new three_1.Vector2();
        ...
        for (let y = 0; y < nmeans; y++) {
            for (let x = 0; x < nmeans; x++) {
                let center = new three_1.Vector2((x + 0.5) / nmeans * gridsize, (y + 0.5) / nmeans * gridsize);
                buckets.push({
                    center: center,
                    bounds: new three_1.Box2(center.clone(), center.clone()),
                    sum: new three_1.Vector2(),
                    runningbounds: new three_1.Box2(),
                    samples: 0
                });
            }
        }
...
exports.ImageDiffGrid = ImageDiffGrid;
function rendermeans(rects, buckets) {
    let cnv = globalThis.cnv;
    if (!cnv) {
    ...
    let parentrect = globalThis.render.canvas.getBoundingClientRect();
...
globalThis.test = async (chunka, levela, levelb = 0, chunkb = chunka) => {
    ...
    return () => {
        ...
        let modelmatrix = new three_1.Matrix4().makeTranslation(chunka.chunkx * mapsquare_1.tiledimensions * chunka.loaded.chunkSize, 0, chunka.chunkz * mapsquare_1.tiledimensions * chunka.loaded.chunkSize).premultiply(chunka.rootnode.matrixWorld);
...
function modelPlacementHash(loc) {
    let hash = 0;
    hash = (0, crc32util_1.crc32addInt)(loc.resolvedlocid, hash);
    hash = (0, crc32util_1.crc32addInt)(loc.rotation, hash);
    hash = (0, crc32util_1.crc32addInt)(loc.type, hash);
    if (loc.placement) {
        if (loc.placement.rotation) {
            hash = (0, crc32util_1.crc32addInt)(loc.placement.rotation[0], hash);
            hash = (0, crc32util_1.crc32addInt)(loc.placement.rotation[1], hash);
            hash = (0, crc32util_1.crc32addInt)(loc.placement.rotation[2], hash);
            hash = (0, crc32util_1.crc32addInt)(loc.placement.rotation[3], hash);
        }
        hash = (0, crc32util_1.crc32addInt)(loc.placement.translateX ?? 0, hash);
        hash = (0, crc32util_1.crc32addInt)(loc.placement.translateY ?? 0, hash);
        hash = (0, crc32util_1.crc32addInt)(loc.placement.translateZ ?? 0, hash);
        hash = (0, crc32util_1.crc32addInt)(loc.placement.scale ?? 0, hash);
        hash = (0, crc32util_1.crc32addInt)(loc.placement.scaleX ?? 0, hash);
        hash = (0, crc32util_1.crc32addInt)(loc.placement.scaleY ?? 0, hash);
        hash = (0, crc32util_1.crc32addInt)(loc.placement.scaleZ ?? 0, hash);
    }
    return hash;
}
class RenderDepsTracker {
    ...
    cachedMetas = [];
    cacheSize = 15;
    constructor(source, config, deps, rendermeta) {
        this.config = config;
        this.deps = deps;
...
exports.RenderDepsTracker = RenderDepsTracker;
