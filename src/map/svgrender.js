...
function mergePoly(set, corners, dirtystart, dirtycount) {
...
async function jsonIcons(engine, locs, rect, maplevel) {
        ...
        if (loc.location.mapFunction) {
            ...
            if (!group) {
                ...
                if (maplabel.sprite != undefined) {
                    let spritefile = await engine.getFileById(constants_1.cacheMajors.sprites, maplabel.sprite);
                    let sprite = (0, sprite_1.parseSprite)(spritefile);
                    src = await (0, imgutils_1.pixelsToDataUrl)(sprite[0].img);
...
async function svgfloor(engine, grid, locs, rect, maplevel, pxpertile, wallsonly, drawicons, thicklines = false) {
    ...
    let maplabels = new Map();
    let overlays = [new Map(), new Map(), new Map(), new Map()];
    let whitelines = [];
    let redlines = [];
    ...
    if (drawground) {
        ...
        for (let dz = 0; dz < rect.zsize; dz++) {
            for (let dx = 0; dx < rect.xsize; dx++) {
                ...
                let occluded = false;
                for (let level = mapsquare_1.squareLevels - 1; !occluded && level >= 0; level--) {
                    ...
                    if (tile.waterProps || tile.rawOverlay) {
                        ...
                        if (vertices.length != 0) {
                            let set = (0, utils_1.getOrInsert)(overlays[level], colint, () => new Set());
        ...
        underlay = await (0, imgutils_1.pixelsToDataUrl)((0, imgutils_1.makeImageData)(underlaybitmap, rect.xsize, rect.zsize));
    }
    ...
    for (let loc of locs) {
        ...
        let occluded = false;
        for (let level = loc.plane + 1; level < mapsquare_1.squareLevels; level++) {
            let tile = grid.getTile(loc.x, loc.z, level);
            if (!tile || tile?.effectiveLevel != loc.effectiveLevel) {
                break;
            }
            if (tile.effectiveLevel == maplevel && (getOverlayColor(tile) != transparent || tile.underlayVisible)) {
                occluded = true;
            }
        }
