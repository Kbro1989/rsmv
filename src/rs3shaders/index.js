...
exports.minimapLocMaterial = minimapLocMaterial;
exports.minimapFloorMaterial = minimapFloorMaterial;
exports.minimapWaterMaterial = minimapWaterMaterial;
...
function minimapLocMaterial(texture, alphamode, alphathreshold) {
    let mat = new three_1.ShaderMaterial();
    mat.uniforms = {
    ...
    if (texture) {
        texture.colorSpace = three_1.SRGBColorSpace;
    }
    return mat;
}
function minimapFloorMaterial(texture) {
    ...
    mat.vertexColors = true;
    let vert = require("./minimap-floor-vert.glsl.c");
    vert = fixShader(vert);
    vert = replaceUniforms(vert, inputreplace);
    vert = replaceDefines(vert, definereplace);
    let frag = require("./minimap-floor-frag.glsl.c");
    frag = fixShader(frag);
    frag = replaceUniforms(frag, inputreplace);
...
function minimapWaterMaterial(texture) {
    ...
    mat.vertexColors = true;
    let vert = require("./minimap-water-vert.glsl.c");
    vert = fixShader(vert);
    vert = replaceUniforms(vert, inputreplace);
    vert = replaceDefines(vert, definereplace);
    let frag = require("./minimap-water-frag.glsl.c");
    frag = fixShader(frag);
    frag = replaceUniforms(frag, inputreplace);
    frag = replaceDefines(frag, definereplace);
    frag = frag.replace(/#undef gl_FragColor/, "// $&");
