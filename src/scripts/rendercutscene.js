...
exports.renderCutscene = renderCutscene;
...
async function renderCutscene(engine, file) {
    let obj = opdecoder_1.parse.cutscenes.read(file, engine);
    ...
    let uuid = `cutscene-${(0, crc32util_1.crc32)(file) >>> 0}`;
    let css = "";
    let html = "";
    ...
    for (let i = obj.elements.length - 1; i >= 0; i--) {
        ...
        if (el.subtitle) {
            html += `<div class="subtitle"><div>${(0, utils_1.escapeHTML)(el.subtitle)}</div></div>\n`;
        }
        if (el.soundid) {
            try {
                let file = await (0, musictrack_1.parseMusic)(engine, constants_1.cacheMajors.sounds, el.soundid, null, true);
        ...
        if (el.graphics) {
            if (el.graphics.length != 0) {
                for (let imgindex = el.graphics.length - 1; imgindex >= 0; imgindex--) {
                    ...
                    if (!pngfile) {
                        let spritebuf = await engine.getFileById(constants_1.cacheMajors.sprites, img.spriteid);
                        pngfile = await (0, imgutils_1.pixelsToDataUrl)((0, sprite_1.parseSprite)(spritebuf)[0].img);
    ...
    function embeddedModule(endtime) {
